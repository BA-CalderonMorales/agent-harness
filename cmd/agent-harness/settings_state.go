...
func (app *App) getSettings() []tui.Setting {
	...
	return []tui.Setting{
		// Provider & Connection
		{Key: "provider", Label: "Provider", Value: app.config.Provider, Category: "Provider & Connection", Description: "API provider (openai, anthropic, openrouter, ollama, flm, fireworks, nvidia, omniroute, local).", Type: "choice", Options: []string{"local", "openai", "anthropic", "openrouter", "ollama", "flm", "fireworks", "nvidia", "omniroute"}},
