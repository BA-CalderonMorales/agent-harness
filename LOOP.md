...
## Active Outcome
**002: Tool outputs are automatically truncated to safe limits, preventing context window overflow and preserving agent loop stability**
> Agent harness truncates bash and file-read outputs before they reach the LLM, matching Claude Code's behavior of silently protecting context budget.
...
| 002 | output-truncation | If we cap bash and read outputs at ~12k chars / ~300 lines, the agent loop will not crash from context overflow on large files or verbose commands. | active |
